...
public class ResearchCloakActionTest {
  @Test
  public void test_() {
    ...
    Action<Character> research;
    // check resource
    parse = new ActionParser("rcloak", null, null, 5, 0, 1);
    research = new ResearchCloakAction<>();
    result = research.doAction(parse, map , p1);
    assertEquals("That action is invalid: The technology resources are not enough for researching cloaking.", result);
    p1.updateResource();
    assertEquals(1002, p1.getWoodNum());
    assertEquals(false, p1.cloakIsResearch());

    // valid 
    parse = new ActionParser("rcloak", null, null, 5, 0, 5);
    research = new ResearchCloakAction<>();
    result = research.doAction(parse, map , p1);
    assertEquals(902, p1.getWoodNum());
    assertEquals(true, p1.cloakIsResearch());
  }
